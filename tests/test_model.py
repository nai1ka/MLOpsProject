...
@patch(
    ...
    read_data="model_name: test_model\nmodel_version: 1.0",
)
def test_read_model_meta(mock_open, mock_path_join):
    ...
    mock_path_join.return_value = os.path.join(
        PROJECTPATH, "models", model_alias, "registered_model_meta"
    )

    model_name, model_version = read_model_meta(model_alias)

    mock_open.assert_called_once_with(
        os.path.join(PROJECTPATH, "models", model_alias, "registered_model_meta"), "r"
    )
