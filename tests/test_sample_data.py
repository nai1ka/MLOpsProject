...
def test_sample_data_creates_sample(config):
    ...
    assert os.path.exists(
        PROJECTPATH + "/data/samples/sample.csv"
    ), "Sample file was not created"
...
def test_sample_data_correct_sample_size(config):
    ...
    assert (
        len(sample_df) == expected_size
    ), f"Sample size is incorrect. Expected {expected_size}, got {len(sample_df)}"
