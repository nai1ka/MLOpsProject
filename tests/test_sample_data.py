...
def test_sample_data_correct_sample_size(config):
    ...
    df = pd.read_csv("../data/" + config.data.dataset_name)
    sample_df = pd.read_csv("../data/samples/" + config.data.output_name)

    # Check the sample size
    expected_size = int(config.data.sample_size * len(df))
