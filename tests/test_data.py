...
from src.data import (
    extract_data,
    check_and_impute_datetime,
    transform_data,
    validate_features,
    save_features_target,
    get_artifact,
    extract_features,
)
...
def test_extract_data(mock_compose, mock_initialize, mock_read_csv, mock_get_url):
    ...
    mock_compose.return_value = MagicMock(
        sample_version="v1", data_path="data/path", data_store="remote_store"
    )
    ...
    mock_get_url.assert_called_once_with(
        ...
        path="data/path",
        remote="remote_store",
        repo=os.path.expandvars("$PROJECTPATH"),
    )
...
def test_transform_data(config):
    ...
    assert (
        X_transformed.shape[1] > df.shape[1]
    )  # Check that the number of columns has increased due to transformations
...
def test_validate_features(mock_get_artifact, MockFileDataContext, config):
    ...
    mock_context.get_validator.return_value.expect_column_values_to_be_between.assert_any_call(
        "hour", min_value=0, max_value=24
    )
...
def test_get_artifact(mock_Client):
    ...
    mock_client.list_artifact_versions.return_value.items = [
        MagicMock(
            created="2024-07-23T12:00:00", load=lambda: pd.DataFrame({"data": [1]})
        )
    ]
