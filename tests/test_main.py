...
def test_run(mock_extract_features, mock_train, mock_evaluate, mock_log_metadata):
    ...
    _, evaluate_kwargs = mock_evaluate.call_args
    assert evaluate_kwargs["evaluate_saved_model"] is False
    assert evaluate_kwargs["model"] == mock_gs

    assert set(tuple(row) for row in evaluate_kwargs["X_test"]) <= set(
        tuple(row) for row in [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    )
