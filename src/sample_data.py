...
def sample_data(cfg: DictConfig = None):
    ...
    if not os.path.exists(f"{PROJECTPATH}/data/{dataset_name}"):
        print("File doesn't exist. Downloading...")
        df = pd.read_csv(cfg.google_drive_url)
        df.to_csv(f"{PROJECTPATH}/data/{dataset_name}", index=False)
    else:
        df = pd.read_csv(f"{PROJECTPATH}/data/{dataset_name}")
